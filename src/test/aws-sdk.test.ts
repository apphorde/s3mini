...
describe("AWS SDK v3 compatibility", () => {
  ...
  it("supports SDK bucket and object CRUD", async () => {
    ...
    await client.send(
      new PutObjectCommand({ Bucket: bucket, Key: "a.txt", Body: "a" }),
    );
    await client.send(
      new PutObjectCommand({ Bucket: bucket, Key: "b.txt", Body: "b" }),
    );
    ...
    const object = await client.send(
      new GetObjectCommand({ Bucket: bucket, Key: "sdk.txt" }),
    );
    expect(await object.Body?.transformToString()).toBe("hello SDK");
    const head = await client.send(
      new HeadObjectCommand({ Bucket: bucket, Key: "sdk.txt" }),
    );
    ...
    const checksum = crypto
      ...
      .update(checksumBody)
      .digest("base64");
    const checksumPut = await client.send(
      new PutObjectCommand({
        Bucket: bucket,
        Key: "checksum.txt",
        Body: checksumBody,
    ...
    const copied = await client.send(
      new CopyObjectCommand({
        Bucket: bucket,
        CopySource: `${bucket}/sdk.txt`,
        Key: "copied.txt",
      }),
    );
    expect(copied.CopyObjectResult?.ETag).toBeTruthy();
    const copiedObject = await client.send(
      new GetObjectCommand({ Bucket: bucket, Key: "copied.txt" }),
    );
    expect(await copiedObject.Body?.transformToString()).toBe("hello SDK");
    const copiedHead = await client.send(
      new HeadObjectCommand({ Bucket: bucket, Key: "copied.txt" }),
    );
    expect(copiedHead.Metadata).toEqual({ color: "blue" });

    await client.send(
      new DeleteObjectCommand({ Bucket: bucket, Key: "sdk.txt" }),
    );
    ...
    expect(batchDelete.Deleted?.map((item) => item.Key)).toEqual([
      "a.txt",
      "b.txt",
    ]);
    await client.send(
      new DeleteObjectCommand({ Bucket: bucket, Key: "copied.txt" }),
    );
    await client.send(
      new DeleteObjectCommand({ Bucket: bucket, Key: "checksum.txt" }),
    );
    await client.send(
      new DeleteObjectCommand({ Bucket: bucket, Key: "already-missing.txt" }),
    );
  ...
  it("authenticates using a persisted access key", async () => {
    ...
    delete process.env.S3MINI_ACCESS_KEY;
    delete process.env.S3MINI_SECRET_KEY;
    const persistedClient = new S3Client({
      endpoint,
      region: "local",
      forcePathStyle: true,
      credentials,
  ...
  it("supports SDK multipart upload commands", async () => {
    ...
    const object = await client.send(
      new GetObjectCommand({ Bucket: bucket, Key: "multipart.txt" }),
    );
  ...
  it("supports SDK tagging and versioning commands", async () => {
    ...
    await client.send(
      new PutBucketTaggingCommand({
        Bucket: bucket,
        Tagging: { TagSet: [{ Key: "team", Value: "storage" }] },
      }),
    );
    expect(
      (await client.send(new GetBucketTaggingCommand({ Bucket: bucket })))
        .TagSet,
    ).toEqual([{ Key: "team", Value: "storage" }]);
    ...
    await client.send(
      new PutObjectTaggingCommand({
        Bucket: bucket,
        Key: "tagged.txt",
        Tagging: { TagSet: [{ Key: "kind", Value: "test" }] },
      }),
    );
    expect(
      (
        await client.send(
          new GetObjectTaggingCommand({ Bucket: bucket, Key: "tagged.txt" }),
        )
      ).TagSet,
    ).toEqual([{ Key: "kind", Value: "test" }]);
    await client.send(
      new DeleteObjectTaggingCommand({ Bucket: bucket, Key: "tagged.txt" }),
    );
  });
});
