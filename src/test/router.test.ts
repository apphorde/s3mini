...
describe("S3 HTTP routes", () => {
  ...
  it("covers bucket listing, location, tags, and configuration deletion", async () => {
    await app.inject({
      method: "PUT",
      url: `/${bucket}?locationConstraint=eu-west-1`,
    });
    ...
    expect(
      (
        await app.inject({
          ...
          payload: JSON.stringify({ CannedACL: "private" }),
        })
      ).statusCode,
    ).toBe(200);
    expect(
    ...
    expect(
      (
        await app.inject({
          ...
          url: `/${bucket}?website`,
          headers: { "content-type": "application/json" },
          payload: JSON.stringify({ index: "index.html" }),
        })
      ).statusCode,
    ).toBe(200);
    expect(
  ...
  it("supports listing, ranges, and conditional requests", async () => {
    ...
    const list = await app.inject({
      method: "GET",
      url: `/${bucket}?delimiter=/&max-keys=1`,
    });
  ...
  it("supports URL-encoded ListObjectsV2 responses", async () => {
    ...
    await app.inject({
      method: "PUT",
      url: `/${bucket}/folder/space%20key.txt`,
      headers: { "content-type": "text/plain" },
      payload: "encoded",
    });
    const list = await app.inject({
      method: "GET",
      url: `/${bucket}?encoding-type=url`,
    });
    expect(list.statusCode).toBe(200);
  ...
  it("supports ListObjects version 1 pagination", async () => {
    ...
    const first = await app.inject({
      method: "GET",
      url: `/${bucket}?list-type=1&max-keys=1`,
    });
    ...
    const second = await app.inject({
      method: "GET",
      url: `/${bucket}?list-type=1&marker=a.txt&max-keys=1`,
    });
  ...
  it("supports bucket versioning and tagging query operations", async () => {
    ...
    const versioning = await app.inject({
      method: "GET",
      url: `/${bucket}?versioning`,
    });
  ...
  it("accepts lifecycle configuration XML over HTTP", async () => {
    await app.inject({ method: "PUT", url: `/${bucket}` });
    const lifecycle =
      "<LifecycleConfiguration><Rule><ID>expire-tmp</ID><Filter><Prefix>tmp/</Prefix></Filter><Status>Enabled</Status><Expiration><Days>0</Days></Expiration></Rule></LifecycleConfiguration>";
  ...
  it("supports multipart uploads over HTTP", async () => {
    ...
    const part = await app.inject({
      method: "PUT",
      url: `/${bucket}/multi.bin?uploadId=${uploadId}&partNumber=1`,
      headers: { "content-type": "application/octet-stream" },
      payload: "multipart",
    });
  ...
  it("lists and aborts multipart uploads over HTTP", async () => {
    ...
    const initiated = await app.inject({
      method: "POST",
      url: `/${bucket}/pending.bin?uploads`,
    });
    const uploadId = initiated.body.match(/<UploadId>([^<]+)<\/UploadId>/)?.[1];
    await app.inject({
      method: "PUT",
      url: `/${bucket}/pending.bin?uploadId=${uploadId}&partNumber=1`,
      headers: { "content-type": "application/octet-stream" },
      payload: "pending",
    });
    const parts = await app.inject({
      method: "GET",
      url: `/${bucket}/pending.bin?uploadId=${uploadId}`,
    });
  ...
  it("validates and returns SHA-256 checksums", async () => {
    ...
    const bad = await app.inject({
      method: "PUT",
      url: `/${bucket}/bad.txt`,
      headers: { "content-type": "text/plain", "x-amz-checksum-sha256": "bad" },
      payload: body,
    });
  ...
  it("validates Content-MD5 and preserves Content-Language", async () => {
    ...
    const bad = await app.inject({
      ...
      headers: { "content-type": "text/plain", "content-md5": "bad" },
      payload: body,
    });
    expect(bad.statusCode).toBe(400);
    expect(bad.body).toContain("<Code>BadDigest</Code>");
  ...
  it("validates AWS and Backblaze payload digest headers", async () => {
    ...
    const bad = await app.inject({
      method: "PUT",
      url: `/${bucket}/bad-digest.txt`,
      headers: { "content-type": "text/plain", "x-bz-content-sha1": "bad" },
      payload: body,
    });
    expect(bad.statusCode).toBe(400);
    expect(bad.body).toContain("<Code>BadDigest</Code>");
  ...
  it("rejects unsupported storage classes", async () => {
    ...
    const response = await app.inject({
      method: "PUT",
      url: `/${bucket}/invalid-class.txt`,
      headers: {
        "content-type": "text/plain",
        "x-amz-storage-class": "MADE_UP",
      },
      payload: "invalid",
    });
  ...
  it("supports restore and basic select object content", async () => {
    ...
    const restored = await app.inject({
      method: "POST",
      url: `/${bucket}/data.csv?restore`,
      headers: { "content-type": "application/xml" },
      payload: "<RestoreObjectRequest />",
    });
    expect(restored.statusCode).toBe(202);
    const selected = await app.inject({
      method: "POST",
      url: `/${bucket}/data.csv?select&select-type=2`,
      headers: { "content-type": "application/xml" },
      payload:
        "<SelectObjectContentRequest><Expression>SELECT * FROM S3Object</Expression></SelectObjectContentRequest>",
    });
  ...
  it("supports UploadPartCopy", async () => {
    ...
    await app.inject({
      method: "PUT",
      url: `/${bucket}/source.bin`,
      headers: { "content-type": "application/octet-stream" },
      payload: "copied-part",
    });
    const initiated = await app.inject({
      method: "POST",
      url: `/${bucket}/destination.bin?uploads`,
    });
    const uploadId = initiated.body.match(/<UploadId>([^<]+)<\/UploadId>/)?.[1];
    const copied = await app.inject({
      method: "PUT",
      url: `/${bucket}/destination.bin?uploadId=${uploadId}&partNumber=1`,
      headers: { "x-amz-copy-source": `/${bucket}/source.bin` },
    });
  ...
  it("enforces policy allows, wildcard resources, and string conditions", async () => {
        ...
        statements: [
          {
            ...
            condition: { StringEquals: { "s3:prefix": "allowed" } },
          },
        ],
      }),
    });
    const allowed = await app.inject({
      method: "GET",
      url: `/${bucket}/allowed.txt?prefix=allowed`,
    });
    expect(allowed.statusCode).toBe(200);
    const denied = await app.inject({
      method: "GET",
      url: `/${bucket}/allowed.txt?prefix=other`,
    });
    expect(denied.statusCode).toBe(403);
  });
  ...
  it("evaluates XML ACL grants for anonymous reads", async () => {
    ...
    await app.inject({
      method: "PUT",
      url: `/${bucket}/grant.txt`,
      headers: { "content-type": "text/plain" },
      payload: "grant",
    });
    const acl =
      "<AccessControlPolicy><AccessControlList><Grant><Grantee><Type>Group</Type><URI>http://acs.amazonaws.com/groups/global/AllUsers</URI></Grantee><Permission>READ</Permission></Grant></AccessControlList></AccessControlPolicy>";
    await app.inject({
      method: "PUT",
      url: `/${bucket}/grant.txt?acl`,
      headers: { "content-type": "application/xml" },
      payload: acl,
    });
    expect(
  ...
  it("protects the access-key control plane with an admin bearer token", async () => {
    ...
    const created = await app.inject({
      ...
      payload: JSON.stringify({ displayName: "control-plane-test" }),
    });
    expect(created.statusCode).toBe(201);
    const credentials = created.json() as {
      accessKeyId: string;
    ...
    expect(
      (
        await app.inject({
          method: "DELETE",
          url: `/admin/access-keys/${credentials.accessKeyId}`,
          headers: { authorization: "Bearer test-admin-token" },
        })
      ).statusCode,
    ).toBe(204);
  ...
  it("manages buckets and policies through the admin control plane", async () => {
    ...
    const created = await app.inject({
      ...
      payload: JSON.stringify({ name: managedBucket }),
    });
    expect(created.statusCode).toBe(201);
    expect(
      (
      ...
      statements: [
        {
          ...
          principal: "*",
          action: "s3:PutObject",
          resource: `arn:aws:s3:::${managedBucket}/*`,
        },
      ],
    };
    expect(
      (
        await app.inject({
          method: "PUT",
          url: `/admin/buckets/${managedBucket}/policy`,
          headers: {
            authorization: "Bearer test-admin-token",
            "content-type": "application/json",
          },
    ...
    expect(
      (
        await app.inject({
          method: "DELETE",
          url: `/admin/buckets/${managedBucket}`,
          headers: { authorization: "Bearer test-admin-token" },
        })
      ).statusCode,
    ).toBe(204);
  ...
  it("exposes dead-letter replication events and supports operator retry", async () => {
    ...
    const listed = await app.inject({
      method: "GET",
      url: "/admin/replication/events?status=DeadLetter",
      headers: { authorization: "Bearer test-admin-token" },
    });
    ...
    expect(
      (
        await app.inject({
          method: "POST",
          url: `/admin/replication/events/${event!.id}/retry`,
          headers: { authorization: "Bearer test-admin-token" },
        })
      ).statusCode,
    ).toBe(204);
  ...
  it("serves the dependency-free control-plane dashboard", async () => {
    ...
    expect(dashboard.body).toContain("<control-plane-app>");
    expect(dashboard.body).toContain('template for="[bucket] of buckets"');
  ...
  it("accepts authenticated replicated object writes without creating a replication loop", async () => {
    ...
    expect(JSON.parse(inventory.body)).toEqual(
      expect.arrayContaining([
