...
const targets = configuredTargets.filter(
  ...
  } => Boolean(target.endpoint && target.accessKeyId && target.secretAccessKey),
);

describe.skipIf(!targets.length)(
  "S3-compatible provider interoperability",
  () => {
        ...
        try {
          ...
          await client.send(
            new PutObjectCommand({
              ...
              Metadata: { client: target.name },
            }),
          );
          const head = await client.send(
            new HeadObjectCommand({ Bucket: bucket, Key: "compat.txt" }),
          );
          ...
          expect(
            await (
              await client.send(
                new GetObjectCommand({ Bucket: bucket, Key: "compat.txt" }),
              )
          ...
          await client.send(
            new DeleteObjectsCommand({
              Bucket: bucket,
              Delete: {
                Objects: [{ Key: "compat.txt" }, { Key: "multipart.bin" }],
              },
            }),
          );
        } finally {
