    ...
    <section class="main">
      ...
      <div class="content">
        ...
        <section class="view active" data-panel="overview">
          <div class="heading"><div><p class="eyebrow" id="date"></p><h1>Overview</h1><p class="muted">A quick look at your object storage infrastructure.</p></div><div class="actions"><button class="button" id="refresh">Refresh</button><button class="button primary" data-view="buckets">+ New bucket</button></div></div>
        ...
        <section class="view" data-panel="buckets"><div class="heading"><div><p class="eyebrow">Workspace</p><h1>Buckets</h1><p class="muted">Create, inspect, and remove storage containers.</p></div></div><div class="panel"><div class="panel-head"><form class="form" id="bucket-form"><input id="bucket-name" placeholder="Bucket name" maxlength="63" required><input id="bucket-region" placeholder="Location (optional)"><button class="button primary">+ Create bucket</button></form><input class="search" id="bucket-search" placeholder="Search buckets"></div><div class="table-wrap"><table><thead><tr><th>Bucket</th><th>Location</th><th>Created</th><th>Policy</th><th></th></tr></thead><tbody id="bucket-table"></tbody></table></div></div></section>
        <section class="view" data-panel="accounts"><div class="heading"><div><p class="eyebrow">Access</p><h1>Accounts</h1><p class="muted">Issue and disable S3 access keys for this node.</p></div></div><div class="panel"><div class="panel-head"><form class="form" id="account-form"><input id="account-name" placeholder="Display name" maxlength="120" required><button class="button primary">+ Issue access key</button></form></div><div class="panel-body"><pre id="issued" class="muted"></pre></div><div class="table-wrap"><table><thead><tr><th>Access key</th><th>Name</th><th>Status</th><th>Created</th><th></th></tr></thead><tbody id="account-table"></tbody></table></div></div></section>
        <section class="view" data-panel="policies"><div class="heading"><div><p class="eyebrow">Governance</p><h1>Policies</h1><p class="muted">Review and update bucket policy documents.</p></div></div><div id="policy-list" class="grid"></div></section>
        <section class="view" data-panel="settings"><div class="heading"><div><p class="eyebrow">Configuration</p><h1>Settings</h1><p class="muted">Replication health and durable event state.</p></div></div><div class="grid"><div class="panel"><div class="panel-head"><div><h2>Peers</h2><p class="muted">Authenticated asynchronous replication</p></div></div><div class="table-wrap"><table><thead><tr><th>Peer</th><th>Status</th><th>Failures</th></tr></thead><tbody id="peer-table"></tbody></table></div></div><div class="panel"><div class="panel-head"><div><h2>Events</h2><p class="muted">Recent replication work</p></div></div><div class="table-wrap"><table><thead><tr><th>Object</th><th>Status</th><th>Attempts</th></tr></thead><tbody id="event-table"></tbody></table></div></div></div></section>
      </div>
    </section>
  </div>
  <script>
    ...
    function render() {
      ...
      document.querySelector('#overview-buckets').innerHTML = state.buckets.slice(0, 5).map(bucket => '<tr><td><strong>' + esc(bucket.name) + '</strong></td><td>' + esc(bucket.locationConstraint || 'us-east-1') + '</td><td>' + esc(new Date(bucket.creationDate).toLocaleString()) + '</td><td><span class="status">Healthy</span></td></tr>').join('') || empty('No buckets yet', 4);
      document.querySelector('#bucket-table').innerHTML = filtered.map(bucket => '<tr><td><strong>' + esc(bucket.name) + '</strong></td><td>' + esc(bucket.locationConstraint || 'us-east-1') + '</td><td>' + esc(new Date(bucket.creationDate).toLocaleString()) + '</td><td><button class="button" data-policy="' + esc(bucket.name) + '">Edit policy</button></td><td><button class="button danger" data-delete-bucket="' + esc(bucket.name) + '">Delete</button></td></tr>').join('') || empty('No matching buckets', 5);
      ...
      document.querySelector('#peer-table').innerHTML = state.peers.map(peer => '<tr><td><code>' + esc(peer.peer) + '</code></td><td><span class="status ' + (peer.status === 'Healthy' ? '' : 'warn') + '">' + esc(peer.status) + '</span></td><td>' + esc(peer.consecutiveFailures) + '</td></tr>').join('') || empty('No configured peers', 3);
      document.querySelector('#event-table').innerHTML = state.events.slice(0, 20).map(event => '<tr><td><code>' + esc(event.bucket + '/' + event.key) + '</code></td><td>' + esc(event.status) + '</td><td>' + esc(event.attempts) + '</td></tr>').join('') || empty('No replication events', 3);
    }
    async function load() {
      try {
    ...
    document.querySelector('#account-form').onsubmit = async event => { event.preventDefault(); const response = await request('/admin/access-keys', { method: 'POST', headers: {'content-type': 'application/json'}, body: JSON.stringify({ displayName: document.querySelector('#account-name').value }) }); if (!response.ok) return status('Unable to issue access key (' + response.status + ').'); const issued = await response.json(); document.querySelector('#issued').textContent = 'Access key: ' + issued.accessKeyId + '\nSecret: ' + issued.secretAccessKey; event.target.reset(); await load(); };
    document.addEventListener('click', async event => { const button = event.target.closest('[data-delete-bucket],[data-disable-key],[data-policy]'); if (!button) return; if (button.dataset.deleteBucket) { if (!confirm('Delete bucket ' + button.dataset.deleteBucket + '?')) return; await request('/admin/buckets/' + encodeURIComponent(button.dataset.deleteBucket), { method: 'DELETE' }); await load(); } if (button.dataset.disableKey) { await request('/admin/access-keys/' + encodeURIComponent(button.dataset.disableKey), { method: 'DELETE' }); await load(); } if (button.dataset.policy) { show('policies'); status('Policy editor for ' + button.dataset.policy + ' is available through the API.'); } });
    load();
  </script>
</body>
</html>`;
