...
<head>
  ...
  <template component="control-plane-app">
    <script setup>
      ...
      const view = ref(['overview', 'buckets', 'accounts', 'users'].includes(requestedView) ? requestedView : 'overview');
      ...
      const request = async (url, options = {}) => {
        ...
        if (response.status === 401 && location.pathname !== '/admin/login') location.href = '/admin/login';
        return response;
      };
      const readJson = async (url, fallback) => { const response = await request(url); return response.ok ? response.json() : fallback; };
      const loadUsers = async () => { try { const response = await request('/admin/users'); if (!response.ok) return setStatus('Unable to load users (' + response.status + ').'); users.value = await response.json(); } catch { setStatus('Unable to load users.'); } };
      const setView = next => { view.value = next; menuOpen.value = false; const url = new URL(location.href); url.searchParams.set('page', next); history.replaceState(null, '', url); if (next === 'users') void loadUsers(); };
      ...
      const setNewUserId = value => { newUserId.value = value; };
      const setNewUserEmail = value => { newUserEmail.value = value; };
      const setNewUserName = value => { newUserName.value = value; };
      const setNewUserRole = value => { newUserRole.value = value; };
      ...
      const load = async () => {
        ...
        profile.value = { name: profileName, email: profileData.email || '', initials: profileName.split(/\\s+/).map(part => part[0]).join('').slice(0, 2).toUpperCase(), meUrl: profileData.meUrl || '', role: profileData.role || '' };
        const nextPolicies = {};
      ...
      const createBucket = async event => {
        ...
        bucketName.value = ''; bucketRegion.value = ''; setView('buckets'); await load();
      };
      ...
      const saveUserRole = async event => { event.preventDefault(); const userId = newUserId.value.trim(); if (!userId) return setStatus('OIDC user ID is required.'); const response = await request('/admin/users/' + encodeURIComponent(userId), { method: 'PUT', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ email: newUserEmail.value, name: newUserName.value, role: newUserRole.value }) }); if (!response.ok) return setStatus('Unable to save user role (' + response.status + ').'); newUserId.value = ''; newUserEmail.value = ''; newUserName.value = ''; await loadUsers(); setStatus('User role saved.'); };
      const removeUserRole = async userId => { const response = await request('/admin/users/' + encodeURIComponent(userId), { method: 'DELETE' }); if (response.ok) location.reload(); else setStatus('Unable to remove user role (' + response.status + ').'); };
      ...
      onInit(load);
      return { view, menuOpen, status, profile, buckets, keys, peers, events, policies, issued, users, newUserId, newUserEmail, newUserName, newUserRole, visibleBuckets, policyDialog, setView, setMenuOpen, setStatus, setBucketName, setBucketRegion, setAccountName, setNewUserId, setNewUserEmail, setNewUserName, setNewUserRole, setSearch, setPolicy, createBucket, issueKey, saveUserRole, removeUserRole, savePolicy, clearPolicy, disableKey, deleteBucket, copy, openPolicy, closePolicy, saveDialogPolicy }; }
    </script>
...
<body>
  ...
  .replace(
    "</nav>",
    `<template if="profile.role === 'admin'"><button bind-class="view === 'users' ? 'block w-full rounded-lg bg-blue-50 px-3 py-2.5 text-left text-sm font-medium text-blue-700' : 'block w-full rounded-lg px-3 py-2.5 text-left text-sm font-medium text-slate-600 hover:bg-slate-50'" on-click="setView('users')">Users</button></template></nav>`,
  )
  .replace(
  ...
  .replace(
    ...
    `<template if="view === 'users'"><section><div class="mb-6"><h1 class="text-2xl font-semibold tracking-tight">Users</h1><p class="mt-1 text-sm text-slate-500">Assign replicated dashboard roles by the user's OIDC subject ID.</p></div><form class="mb-6 grid gap-3 rounded-xl border border-slate-200 bg-white p-5 md:grid-cols-2" on-submit="saveUserRole($event)"><label class="text-xs font-medium text-slate-600">OIDC subject ID<input class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" placeholder="User subject ID" bind-value="newUserId" on-input="setNewUserId($event.target.value)" required></label><label class="text-xs font-medium text-slate-600">Email<input class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" placeholder="user@example.com" bind-value="newUserEmail" on-input="setNewUserEmail($event.target.value)"></label><label class="text-xs font-medium text-slate-600">Name<input class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" placeholder="Display name" bind-value="newUserName" on-input="setNewUserName($event.target.value)"></label><label class="text-xs font-medium text-slate-600">Role<select class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" bind-value="newUserRole" on-change="setNewUserRole($event.target.value)"><option value="viewer">Viewer</option><option value="operator">Operator</option><option value="admin">Admin</option></select></label><div class="md:col-span-2"><button class="rounded-lg bg-blue-600 px-3.5 py-2 text-sm font-medium text-white">Save role</button></div></form><div class="overflow-hidden rounded-xl border border-slate-200 bg-white"><table class="w-full text-left text-sm"><thead class="bg-slate-50 text-xs text-slate-500"><tr><th class="p-3">User</th><th class="p-3">Subject ID</th><th class="p-3">Role</th><th class="p-3">Action</th></tr></thead><tbody><template for="[user] of users"><tr class="border-t border-slate-100"><td class="p-3">{{ user.name || user.email || user.userId }}</td><td class="p-3 font-mono text-xs">{{ user.userId }}</td><td class="p-3 capitalize">{{ user.role }}</td><td class="p-3"><button class="text-red-600" on-click="removeUserRole(user.userId)">Remove</button></td></tr></template></tbody></table><p if="users.length === 0" class="p-5 text-sm text-slate-500">No explicitly assigned user roles.</p></div></section></template><bucket-policy-dialog`,
  )
  .replace(
    '<p if="users.length === 0" class="p-5 text-sm text-slate-500">No explicitly assigned user roles.</p>',
    '<template if="users.length === 0"><p class="p-5 text-sm text-slate-500">No explicitly assigned user roles.</p></template>',
  )
  .replace(
