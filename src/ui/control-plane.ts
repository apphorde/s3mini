...
<head>
  ...
  <template component="bucket-policy-dialog">
    <script setup>
      ...
      export default function () { return { open, bucket, value, update, submit, close, save, noop }; }
    </script>
  ...
  <template component="control-plane-app">
    <script setup>
      ...
      const status = ref('');
      const profile = reactive({ name: 'Administrator', email: '', initials: 'S3' });
      const buckets = ref([]);
      const keys = ref([]);
      const peers = ref([]);
      const events = ref([]);
      const policies = reactive({});
      const policyDialog = reactive({ open: false, bucket: '', value: '{}' });
      const bucketName = ref('');
      const bucketRegion = ref('');
      const accountName = ref('');
      const search = ref('');
      const issued = reactive({ access: '', secret: '', visible: false });
      ...
      const setPolicy = (bucket, value) => { policies[bucket] = value; };
      ...
      const load = async () => {
        ...
        profile.name = profileData.name || profileData.email || 'Administrator';
        profile.email = profileData.email || '';
        profile.initials = profile.name.split(/\\s+/).map(part => part[0]).join('').slice(0, 2).toUpperCase();
        for (const bucket of buckets.value) policies[bucket.name] = JSON.stringify(await readJson('/admin/buckets/' + encodeURIComponent(bucket.name) + '/policy', {}), null, 2);
      };
      ...
      const issueKey = async event => {
        ...
        const value = await response.json(); issued.access = value.accessKeyId; issued.secret = value.secretAccessKey; issued.visible = true; accountName.value = ''; await load();
      };
      const savePolicy = async bucket => {
        try { JSON.parse(policies[bucket]); } catch { return setStatus('Policy JSON is invalid.'); }
        const response = await request('/admin/buckets/' + encodeURIComponent(bucket) + '/policy', { method: 'PUT', headers: { 'content-type': 'application/json' }, body: policies[bucket] });
        setStatus(response.ok ? 'Policy saved for ' + bucket + '.' : 'Unable to save policy (' + response.status + ').');
        if (response.ok) await load();
      };
      const clearPolicy = async bucket => { const response = await request('/admin/buckets/' + encodeURIComponent(bucket) + '/policy', { method: 'DELETE' }); if (response.ok) { policies[bucket] = '{}'; setStatus('Policy cleared for ' + bucket + '.'); } };
      const openPolicy = bucket => { policyDialog.bucket = bucket; policyDialog.value = policies[bucket] || '{}'; policyDialog.open = true; };
      const closePolicy = () => { policyDialog.open = false; };
      const saveDialogPolicy = async event => { try { JSON.parse(event.detail.value); } catch { return setStatus('Policy JSON is invalid.'); } const response = await request('/admin/buckets/' + encodeURIComponent(event.detail.bucket) + '/policy', { method: 'PUT', headers: { 'content-type': 'application/json' }, body: event.detail.value }); if (response.ok) { policyDialog.open = false; await load(); setStatus('Policy saved for ' + event.detail.bucket + '.'); } else setStatus('Unable to save policy (' + response.status + ').'); };
