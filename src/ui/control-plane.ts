  ...
  <div class="shell">
    ...
    <section class="main">
      ...
      <div class="content">
        ...
        <section class="view" data-panel="settings"><div class="heading"><div><p class="eyebrow">Configuration</p><h1>Settings</h1><p class="muted">Authentication, replication health, and durable event state.</p></div></div><div class="panel" style="margin-bottom:20px"><div class="panel-head"><div><h2>OIDC API token</h2><p class="muted">Paste a token issued by the provider's API token endpoint with the <code>s3:admin</code> scope. It stays in this browser session; the database stores only a one-way owner hash.</p></div></div><div class="panel-body"><div class="form"><input id="settings-admin-token" class="token" type="password" placeholder="Provider API token" autocomplete="off"><button id="save-token" class="button primary">Use token</button></div><p id="token-owner" class="muted"></p></div></div><div class="grid"><div class="panel"><div class="panel-head"><div><h2>Peers</h2><p class="muted">Authenticated asynchronous replication</p></div></div><div class="table-wrap"><table><thead><tr><th>Peer</th><th>Status</th><th>Failures</th></tr></thead><tbody id="peer-table"></tbody></table></div></div><div class="panel"><div class="panel-head"><div><h2>Events</h2><p class="muted">Recent replication work</p></div></div><div class="table-wrap"><table><thead><tr><th>Object</th><th>Status</th><th>Attempts</th></tr></thead><tbody id="event-table"></tbody></table></div></div></div></section>
      </div>
    </section>
  </div>
  <script>
    ...
    document.querySelector('#settings-admin-token').value = sessionStorage.getItem('s3mini_admin_token') || '';
