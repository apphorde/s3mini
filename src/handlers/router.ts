...
export async function registerRoutes(
  ...
  fastify.addHook("preValidation", async (request, reply) => {
    ...
    const signingCredentials = credentials
      ? { ...credentials, region: process.env.S3MINI_REGION || "local" }
      : undefined;
    if (
      hasPresign &&
      (!signingCredentials ||
        !verifyPresignedSigV4(
          {
    ...
    if (bearerToken && !hasOidcScope(oidcScopes, requiredOidcScope(request))) {
      throw new S3Error(
        "AccessDenied",
        `The bearer token lacks the required ${requiredOidcScope(request)} scope.`,
        403,
      );
    }
  ...
  fastify.put("/internal/replication", async (request, reply) => {
    ...
    if (operation === "PutObject") {
      ...
      await s3.acceptReplicatedDelete({
        bucket,
        key,
        versionId,
        lastModified,
  ...
  fastify.get("/auth/callback", async (request, reply) => {
    ...
    const redirectUri =
      ...
      process.env.S3MINI_OIDC_REDIRECT_URI ||
      `${requestBaseUrl(request)}/auth/callback`;
    const tokenResponse = await fetch(`${oidcBaseUrl()}/token`, {
      method: "POST",
  ...
  fastify.options("/:bucket/*", async (request, reply) => {
    ...
    const configuration = await s3.getBucketConfiguration<{
  ...
  async function putObject(request: FastifyRequest, reply: FastifyReply) {
    ...
    if (query.uploadId && query.partNumber) {
      ...
      const part = await s3.uploadPart({
        ...
        partNumber: Number(query.partNumber),
    ...
    if (copySource) {
      ...
      return reply
        ...
        .header("x-amz-version-id", obj.versionId)
        .send(
          wrapXml("CopyObjectResult", {
            ETag: obj.etag,
    ...
    if (contentMd5 && String(contentMd5) !== contentMd5Digest) {
      throw new S3Error(
        "BadDigest",
        "The Content-MD5 checksum did not match the request body.",
        400,
        params.bucket,
        key,
      );
    ...
    meta.userMetadata = Object.fromEntries(
      Object.entries(request.headers)
        ...
        .map(([name, value]) => [
          name.slice("x-amz-meta-".length).toLowerCase(),
    ...
    if (encryption === "aws:kms" && !kmsKeyId) {
      throw new S3Error(
        "InvalidRequest",
        "A KMS key identifier is required for aws:kms encryption.",
        400,
        params.bucket,
        key,
      );
  ...
  async function getObject(request: FastifyRequest, reply: FastifyReply) {
    ...
    if (
      ...
      !String(ifMatch)
        ...
        .includes(original.metadata.etag)
    ) {
      throw new S3Error(
        "PreconditionFailed",
    ...
    const ifModifiedSince = request.headers["if-modified-since"]
      ? Date.parse(String(request.headers["if-modified-since"]))
      : NaN;
    ...
    if (
      ...
      return reply
        ...
        .header("Last-Modified", original.metadata.lastModified.toUTCString())
        .send();
    }
  ...
  fastify.post("/:bucket/*", async (request, reply) => {
    ...
    reply
      ...
      .header("x-amz-version-id", result.versionId)
      .send(
        wrapXml("CompleteMultipartUploadResult", {
          Bucket: result.bucket,
          Key: result.key,
          ETag: result.etag,
...
function requiredOidcScope(request: FastifyRequest): string {
  ...
  if (
    query.policy !== undefined ||
    query.acl !== undefined ||
    query.encryption !== undefined ||
    query.website !== undefined ||
...
function parseAclXml(body: string): Record<string, unknown> {
  const grants = [
    ...body.matchAll(
      /<Grant>\s*<Grantee(?:\s+[^>]*)?>([\s\S]*?)<\/Grantee>\s*<Permission>([^<]+)<\/Permission>\s*<\/Grant>/g,
    ),
  ].map((match) => {
