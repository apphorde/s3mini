...
export function verifySigV4(
  ...
  if (
    ...
    service !== (credentials.service || "s3") ||
  ...
  if (
    ...
    timestamp.slice(0, 8) !== date
  )
    return false;
  const requestTime = parseTimestamp(timestamp);
  const now = (request.now || new Date()).getTime();
  if (!Number.isFinite(requestTime) || Math.abs(now - requestTime) > 900_000)
    return false;

  const payloadHash =
    first(request.headers["x-amz-content-sha256"]) || "UNSIGNED-PAYLOAD";
  const canonicalHeaders = signedHeaders
    .map((name) => `${name}:${normalizeHeader(request.headers[name])}\n`)
    .join("");
...
export function verifyPresignedSigV4(
  request: SigV4Request,
  credentials: SigV4Credentials,
): boolean {
  ...
  if (
    credential.length !== 5 ||
    credential[0] !== credentials.accessKeyId ||
    credential[2] !== credentials.region ||
    credential[3] !== (credentials.service || "s3") ||
  ...
  if (
    ...
    now < signedAt - 300_000 ||
    now > signedAt + expires * 1000
  )
    return false;
  ...
  const canonicalRequest = [
    request.method.toUpperCase(),
    canonicalPath(request.url),
    canonicalQuery(request.url, true),
    canonicalHeaders,
    signedHeaders,
    "UNSIGNED-PAYLOAD",
  ].join("\n");
  ...
  const signingKey = hmac(
    hmac(
      hmac(
        hmac(`AWS4${credentials.secretAccessKey}`, credential[1]),
        credential[2],
      ),
      credential[3],
    ),
...
function canonicalQuery(url: string, omitSignature = false): string {
  ...
  return [...query.entries()]
    .filter(([key]) => !omitSignature || key !== "X-Amz-Signature")
